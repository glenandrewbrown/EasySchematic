...
import { useSchematicStore, isLayerEffectivelyHiddenIn, isLayerEffectivelyLockedIn } from "../store";
...
export default function LayersPanel({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const setLayerParent = useSchematicStore((s) => s.setLayerParent);
  ...
  // Drag-to-reparent state for layer rows themselves (nested layer groups).
  const [dragLayerId, setDragLayerId] = useState<string | null>(null);
  // Layer header currently a valid drop target for a layer-row drag (reparent), or null.
  const [dropLayerParentId, setDropLayerParentId] = useState<string | null>(null);
  ...
  // Render one layer row, recursing into nested sub-layers (SchematicLayer.parentId,
  // e.g. Audio ▸ Speakers ▸ Kiis) before the layer's own group/node members.
  const renderLayerRow = (layer: LayerTreeNode, depth: number): ReactElement => {
    const layerNodeIds = collectNodeIds(layer);
    const subLayers = layer.subLayers ?? [];
    const isOpen = expanded.has(layer.id);
    const isSolo = soloLayerId === layer.id;
    const isBase = layer.id === DEFAULT_LAYER_ID;
    const effHidden = isLayerEffectivelyHiddenIn(layers, layer.id);
    const effLocked = isLayerEffectivelyLockedIn(layers, layer.id);
    const inheritedHidden = effHidden && layer.visible;
    const inheritedLocked = effLocked && !layer.locked;
      ...
      <div key={layer.id}>
        <div
          ...
          onDragOver={(e) => {
            ...
            if (dragLayerId != null && dragLayerId !== layer.id) {
              e.preventDefault();
              e.dataTransfer.dropEffect = "move";
              if (dropLayerParentId !== layer.id) setDropLayerParentId(layer.id);
            }
          }}
          onDragLeave={() => {
            setDropLayerId((id) => (id === layer.id ? null : id));
            setDropLayerParentId((id) => (id === layer.id ? null : id));
          }}
          onDrop={(e) => {
            e.preventDefault();
            e.stopPropagation();
            if (dragNodeId != null) {
              ...
              setDropTarget(null);
              setDropLayerId(null);
              if (dragged == null || effLocked) return;
            ...
            const draggedLayer = dragLayerId;
            setDragLayerId(null);
            setDropLayerParentId(null);
            if (draggedLayer != null && draggedLayer !== layer.id) {
              setLayerParent(draggedLayer, layer.id);
            }
          }}
          className={`flex items-center gap-1 pr-1.5 py-1.5 rounded-md group hover:bg-[var(--color-surface-hover)] ${effHidden ? "opacity-50" : ""} ${dragLayerId === layer.id ? "opacity-40" : ""}`}
          style={{
            paddingLeft: pad,
            ...(dropLayerParentId === layer.id
        ...
        >
          <button
            ...
            className={`cursor-pointer shrink-0 ${layer.children.length === 0 && subLayers.length === 0 ? "opacity-20 pointer-events-none" : "text-[var(--color-text-muted)]"}`}
            title={isOpen ? "Collapse" : "Expand"}
          >
            <Chevron open={isOpen} />
          </button>
          <button
            ...
            title={inheritedHidden ? "Hidden by a parent layer" : layer.visible ? "Hide layer" : "Show layer"}
          >
            <EyeIcon off={effHidden} />
          </button>
          <button
            ...
            className={`cursor-pointer shrink-0 ${
              layer.locked
                ...
                : inheritedLocked
                  ? "text-[var(--color-warning)] opacity-40"
                  : "text-[var(--color-text-muted)] opacity-40 group-hover:opacity-100"
            }`}
            title={inheritedLocked ? "Locked by a parent layer" : layer.locked ? "Unlock layer" : "Lock layer"}
          >
            <PadlockIcon open={!effLocked} />
          </button>
          <button
        ...
        {isOpen && (
          <>
            {subLayers.map((sl) => renderLayerRow(sl, depth + 1))}
            {layer.children.map((c) => renderChild(c, depth + 1))}
          </>
        )}
      </div>
    );
  ...
  return (
    <div className={`${embedded ? "" : "w-56 bg-[var(--color-surface)] border-l border-[var(--color-border)] "}flex flex-col h-full overflow-hidden`} data-print-hide>
      ...
      <div
        ...
        onDragOver={(e) => {
          // Empty space below the tree = drop-to-root: only relevant while
          // dragging a layer row (unparents it back to the top level).
        ...
        onDrop={(e) => {
          e.preventDefault();
          if (dragLayerId == null) return;
          setLayerParent(dragLayerId, null);
          setDragLayerId(null);
          setDropLayerParentId(null);
        }}
      >
        {tree.map((layer) => renderLayerRow(layer, 0))}
      </div>
      ...
      <div className="p-1.5 border-t border-[var(--ui-border)]">
        ...
        {hasSelection && (
          <p className="text-[10px] text-[var(--color-text-muted)] px-1.5 pb-1">
            Hover a layer and click → to move the selection there. Drag a layer
            onto another to nest it; drop on empty space to un-nest.
          </p>
        )}
      </div>
    </div>
